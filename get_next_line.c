...
/*   Updated: 2024/12/10 13:49:23 by zmeliani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (!text_buff)
	{
		text_buff = malloc(sizeof(char));
		if (!text_buff)
			return (free(text_buff), NULL);
		text_buff[0] = 0;
	}
	text_buff = read_text(fd, text_buff);
	line = line_text(text_buff);
	text_buff = trim_text(text_buff, line);
	return (line);
}
